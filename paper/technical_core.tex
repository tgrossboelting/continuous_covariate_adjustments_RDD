...
\begin{remark}[Connection to AK's worst-case bias]\label{rem:ak_support_function}
	For Lipschitz classes ($p = 1$), the worst-case bias in \cref{prop:wcb} is a direct instance of the framework in \textcite{armstrong_optimal_2018}.
	Their modulus under centrosymmetry (\citeyear{armstrong_optimal_2018}, eq.~25),
	\[
		\omega(\delta; \mathcal{F}) = \sup\bigl\{ 2Lf : \norm{Kf} \leq \delta/2,\; f \in \mathcal{F} \bigr\},
	\]
	is itself a support function computation: it maximizes a linear functional ($2Lf$) over the convex set $\mathcal{F} \cap \{f : \norm{Kf} \leq \delta/2\}$.
	For gradient-constrained Lipschitz classes $\mathcal{F}(\mathcal{C})$, this infinite-dimensional optimization reduces to the finite-dimensional support function $h_{\mathcal{C}}$ because the bias is \emph{directly linear} in the gradient:
	$f(x_i) - f(x_0) = \int_0^1 \langle \nabla f(x_0 + t(x_i - x_0)),\, x_i - x_0 \rangle \, dt$,
	and maximizing $\langle \nabla f, v \rangle$ over $\nabla f \in \mathcal{C}$ is the definition of $h_{\mathcal{C}}(v)$.
	No intermediate reduction is needed --- the support function characterization is essentially definitional for $p = 1$.

	For second-order smoothness classes ($p = 2$), such as the Hessian-constrained classes in \textcite{imbens_optimized_2019}, the analogous reduction requires an additional step.
	The constraint set $\mathcal{C}_H$ acts on second derivatives, and the bias involves second-order terms:
	$f(x_i) - f(x_0) - \nabla f(x_0) \cdot (x_i - x_0) = \tfrac{1}{2}(x_i - x_0)^\prime H (x_i - x_0) + o(\norm{x_i - x_0}^2)$.
	The worst-case bias is still a support function, but of $\mathcal{C}_H$ evaluated at displacement vectors constructed from the outer products $(x_i - x_0)(x_i - x_0)^\prime$.
	The key step is showing that the bias is linear in the Hessian entries $H_{jk}$: the first-order terms are unconstrained (absorbed by the estimator weights satisfying $\sum w_i = 1$, $\sum w_i x_i = x_0$), leaving only the second-order remainder, which depends linearly on $H$.
\end{remark}
