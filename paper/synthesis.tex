...
\begin{definition}[Primitive-consistent constraint set]\label{def:primitive_consistent}
	Let $M_x, M_z > 0$.
	A set $\mathcal{C} \subset \mathbb{R}^2$ is \emph{primitive-consistent} with bounds $(M_x, M_z)$ if $\mathcal{C}$ is nonempty, convex, centrosymmetric, and satisfies: for all $(u, v) \in \mathcal{C}$, $\abs{u} \leq M_x$ and $\abs{v} \leq M_z$. Note that boundedness of the set is implied by primitive consistency. To facilitate proofs I also require closedness, to ensure the set is compact. This is not strictly necessary, and could be relaxed.
\end{definition}
